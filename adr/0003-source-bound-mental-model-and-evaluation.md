...
- **Scope:** `.yawn` protocol grammar and conformance artifacts; no production runtime or publication
...
This passage is explicitly noncanonical. Stages may be absent, repeated, held, nested, disputed, or branched. The renderer must not fabricate missing stages to make the sequence look complete.

### 3. Position is the canonical record; “Answer” is a human-facing label
...
- criteria snapshot and the basis of each criterion;
...
Move selection uses the orientation and predicted fit available at selection time. Consequences are recorded only after an executed move or other source-backed event. Evaluation may then assess the target against explicit criteria. Proof adjudication checks named evidence against a prior proof condition. An authorized Update may follow.
...
A disclosure requirement or public-expression intention remains intent until the affected owners, principals, audience, consent, redaction, and publication authorization are explicit.
...
Rejected for this draft. Cognition, passage, Evaluation, and governance are modular so reviewers can challenge one boundary without treating the entire aggregate as one indivisible ontology.

## Consequences

### Benefits

- Preserves the useful simplicity of Question / Answer / Expression / Move / Judgment while retaining protocol precision.
...
- Maintaining Position and Evaluation as draft types may overlap with future stable contracts.

Mitigation requires progressive disclosure, one-question choice architecture, explicit holds, source inspection, low-cost correction, retention limits, and comparisons against simpler source-linked notes.

## Migration and compatibility

No accepted record is rewritten. Existing Consequence, Proof, Projection, View, move-selection, and Orientation contracts remain authoritative in their scopes. Implementations may lazily compile the new package from existing records and may omit absent stages.
