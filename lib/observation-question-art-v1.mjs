...
export const VIEW_FEEDBACK_SCHEMA_VERSION = "yawn.view-feedback.v1";
...
export function normalizeViewFeedback(input) {
  assert(input?.schemaVersion === VIEW_FEEDBACK_SCHEMA_VERSION, "view_feedback_schema_invalid");
  nonEmpty(input.feedbackId, "view_feedback_id_required");
  assert(input.ontologyRole === "proposal_facet", "view_feedback_ontology_role_invalid");
  const assertedBy = normalizeRecordRef(input.assertedBy);
  assert(["actor", "principal"].includes(assertedBy.kind), "view_feedback_actor_invalid");
  assert(["local_handoff", "browser_capture", "synthetic_fixture"].includes(input.source?.coordinateKind), "view_feedback_source_kind_invalid");
  assert(input.source?.platformMessageIdVerified === false, "view_feedback_message_id_must_be_unverified");
  nonEmpty(input.source.coordinate, "view_feedback_coordinate_required");
  nonEmpty(input.source.exactQuote, "view_feedback_quote_required");
  nonEmpty(input.source.capturedAt, "view_feedback_captured_at_required");
  const subjectRef = normalizeRecordRef(input.subjectRef, { kind: "view" });
  assert(Array.isArray(input.expectedExperience) && input.expectedExperience.length > 0, "view_feedback_expectations_required");
  const expectedExperience = input.expectedExperience.map((item) => {
    nonEmpty(item.text, "view_feedback_expectation_text_required");
    nonEmpty(item.assertedBy, "view_feedback_expectation_actor_required");
    assert(item.epistemicStatus === "reported", "view_feedback_expectation_status_invalid");
    return { text: item.text, assertedBy: item.assertedBy, epistemicStatus: "reported" };
  });
  assert(Array.isArray(input.actualViewEvidence) && input.actualViewEvidence.length > 0, "view_feedback_evidence_required");
  const actualViewEvidence = input.actualViewEvidence.map((item) => {
    nonEmpty(item.description, "view_feedback_evidence_description_required");
    assert(item.verificationStatus === "reported_not_independently_verified", "view_feedback_evidence_status_invalid");
    return {
  ...
  assert(Array.isArray(input.divergenceDimensions) && input.divergenceDimensions.length > 0, "view_feedback_dimensions_required");
  assert(new Set(input.divergenceDimensions).size === input.divergenceDimensions.length, "view_feedback_dimensions_duplicate");
  for (const dimension of input.divergenceDimensions) assert(divergenceDimensions.has(dimension), "view_feedback_dimension_invalid");
  assert(Array.isArray(input.preferenceUpdateProposals) && input.preferenceUpdateProposals.length > 0, "view_feedback_preferences_required");
  const preferenceUpdateProposals = input.preferenceUpdateProposals.map((proposal) => {
    nonEmpty(proposal.preferenceProposalId, "view_feedback_preference_id_required");
    nonEmpty(proposal.viewKind, "view_feedback_preference_view_required");
    assert(/^\/[A-Za-z0-9_.~/-]+$/.test(proposal.fieldPath), "view_feedback_preference_path_invalid");
    assert(["set", "reset"].includes(proposal.operation), "view_feedback_preference_operation_invalid");
    if (proposal.operation === "reset") assert(proposal.value === null, "view_feedback_reset_value_must_be_null");
    assert(proposal.status === "proposed", "view_feedback_preference_must_remain_proposed");
    assert(proposal.sourceFeedbackId === input.feedbackId, "view_feedback_preference_source_mismatch");
    nonEmpty(proposal.rationale, "view_feedback_preference_rationale_required");
    return { ...proposal, scopeRef: normalizeRecordRef(proposal.scopeRef), status: "proposed" };
  });
  assert(Array.isArray(input.proofObligations) && input.proofObligations.length > 0, "view_feedback_proof_required");
  const proofObligations = input.proofObligations.map((proof) => {
    nonEmpty(proof.obligationId, "view_feedback_proof_id_required");
    const preconditions = stringArray(proof.preconditions, "view_feedback_preconditions");
    nonEmpty(proof.prediction, "view_feedback_prediction_required");
    const postconditions = stringArray(proof.postconditions, "view_feedback_postconditions");
    nonEmpty(proof.verifier, "view_feedback_verifier_required");
    nonEmpty(proof.falsifier, "view_feedback_falsifier_required");
    assert(proof.status === "waiting", "view_feedback_proof_must_wait");
    return { ...proof, preconditions, postconditions, status: "waiting" };
  });
  assert(input.epistemicStatus === "reported", "view_feedback_epistemic_status_invalid");
  assert(input.universality === "subjective_attributed_evaluation", "view_feedback_universality_invalid");
  assert(input.proposalStatus === "proposed", "view_feedback_must_remain_proposed");
  ...
  ], "view_feedback_boundary");
  nonEmpty(input.createdAt, "view_feedback_created_at_required");
  return {
    schemaVersion: VIEW_FEEDBACK_SCHEMA_VERSION,
...
export function viewFeedbackSha256(feedback) {
  return hash(normalizeViewFeedback(feedback));
}
...
export function serializeObservationQuestionArtView({
  ...
  localArtCandidate: candidateInput = null, viewFeedback: feedbackInput = null,
}) {
  const observation = normalizeObservationState(observationInput);
  ...
  const viewFeedback = feedbackInput === null ? null : normalizeViewFeedback(feedbackInput);
