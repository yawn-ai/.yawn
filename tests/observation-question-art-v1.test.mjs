...
import {
  ...
  normalizeProjectionFeedback,
  projectionFeedbackSha256,
...
async function standaloneValidators() {
  ...
  const names = {
    ...
    projectionFeedback: "projection-feedback.v1.schema.json",
  };
...
test("standalone and stable aggregate schemas accept their closed proposal records", async () => {
  ...
  const aggregateNames = {
    ...
    artBrief: "ArtBriefV1",
    projectionFeedback: "ProjectionFeedbackV1",
  };
...
test("Projection feedback remains subjective, attributed, actionable, and sanitized", async () => {
  const { chain } = await records();
  const feedback = normalizeProjectionFeedback(chain.projectionFeedback);
  assert.equal(feedback.assertedBy.id, "principal:fixture");
  assert.equal(feedback.source.coordinate, "fixture://synthetic/projection-feedback#reported-gap");
  ...
  assert.match(projectionFeedbackSha256(feedback), /^[a-f0-9]{64}$/);
  assert.equal(JSON.stringify(chain).includes("conversation://current/"), false);
  assert.throws(() => normalizeProjectionFeedback({ ...feedback, universality: "universal_fact" }), /universality_invalid/);
  assert.throws(() => normalizeProjectionFeedback({
    ...feedback,
...
test("browser feedback is captured honestly without claiming a platform message ID", async () => {
  ...
  const browserFeedback = {
    ...chain.projectionFeedback,
    source: {
      ...chain.projectionFeedback.source,
      coordinate: "browser://local/observation#projection-feedback",
      coordinateKind: "browser_capture",
      platformMessageIdVerified: false,
    },
  };
  const normalized = normalizeProjectionFeedback(browserFeedback);
  assert.equal(normalized.source.coordinateKind, "browser_capture");
  const validate = (await standaloneValidators()).projectionFeedback;
...
test("serialization is a deterministic View materialization, not canonical state", async () => {
  ...
  const input = {
    ...
    projectionFeedback: chain.projectionFeedback,
  };
